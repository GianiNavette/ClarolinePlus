...
if (window.location.pathname.split("/")[1] == "resource") {
  ...
  if (navBar != undefined) {
    //find file id
    var fileId = window.location.pathname.split("/")[4];

    if (!Number.isInteger(fileId)) {
      fileId = window.location.pathname.replace(/\D+/g, "");
    }
    ...
    view.addEventListener("click", function () {
      location.href = "/clarolinepdfplayerbundle/pdf/" + fileId;
    });
    ...
    document
      ...
      .insertAdjacentElement("afterbegin", turn);
    ...
    document
      .querySelector(".pdf-zoom")
      .insertAdjacentElement("afterbegin", turnInv);
